...
describe('engine: rm active-env refusal', () => {
  it('refuses to remove an env bound in a session, unless --drop-first', async () => {
    const th = home();
    await run(['create', 'writing'], { env: th.env });
    const env = { ...th.env, AGENTENV_SESSION: 'S1' };
    await run(['use', 'writing'], { env, cwd: th.home });

    const refused = await run(['rm', 'writing', '--yes'], { env, cwd: th.home });
    expect(refused.code).toBe(1);
    expect(refused.stderr?.toLowerCase()).toContain('active');
    expect(refused.stderr).toContain('--drop-first');
    expect(existsSync(resolvePaths(th.env).envDir('writing'))).toBe(true);

    const dropped = await run(['rm', 'writing', '--drop-first', '--yes'], { env, cwd: th.home });
  ...
  it('refuses to remove an env active in the global stack, unless --drop-first', async () => {
    const th = home();
    const paths = resolvePaths(th.env);
  ...
  it('removes an inactive env without --drop-first (unchanged behaviour)', async () => {
    const th = home();
    await run(['create', 'writing'], { env: th.env });
    const res = await run(['rm', 'writing', '--yes'], { env: th.env });
