...
| Codex | Shared skills, commands-as-skills, child environment, persistence | JJ adapter and tests | Behavioral reimplementation with CM TOML/probe/compiler | Implemented, including field-level reverse persistence and TOML-agent raw mapping |
...
| Pi | Five settings arrays and real probe | CM adapter and fixtures | Existing Apache-2.0 code | Implemented with retained projections and attributable persistence |
| Cursor | Fixed real root, shared skills, explicit unsupported semantics | JJ adapter and tests | Behavioral reimplementation with CM validator | Implemented as global-only with whole-file validation |
| Session generations and inventory | Immutable retained generations and launch-time inventories | JJ view composer/adoption tests | Behavioral reimplementation under new lease model | Implemented with reservations, process-group leases, final sweep, recovery, and conservative GC |
| CLI and release | Settled commands, root globals, env-less global drop, Node floor, container restore | JJ CLI/release tests | Behavioral reimplementation | Implemented; packed global + session restore smoke is green and the Docker gate is wired into CI |
...
| Sync candidates | Isolated fetched tree, last-known-good launch behavior, writer-aware deferral, durable rejection, and status visibility | Merge specification plus CM Git safeguards | New lifecycle around CM validation, secret scan, rebase, and push primitives | Implemented for fetch, validation, WAL promotion, retained-writer deferral, rejection, explicit resolve/abandon, status, and conservative GC |
