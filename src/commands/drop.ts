...
/**
 * `agentenv drop [<env>…] [--global]` — deactivate.
 *
 ...
 *   had shadowed (D5). An env-less `drop --global` clears the whole global stack.
 */
export const dropCommand: Command = {
  ...
  async run(ctx): Promise<RunResult> {
    ...
    if (parsed.booleans.has('global')) {
      return dropGlobal(ctx, parsed.positionals, parsed.positionals.length === 0);
    }
    return dropSession(ctx, parsed.positionals, parsed.positionals.length === 0);
  },
};
...
async function dropSession(
  ...
  // No names → drop the whole binding.
